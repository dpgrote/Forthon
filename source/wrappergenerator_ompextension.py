...
class PyWrap_OMPExtension():
    def OMPInit(self, ompactive=False, ompdebug=False):
        ...
        print('{color} Adding OPENMP variables and routines for package: {}{reset}'.format(
            self.pkgname, color=Back.CYAN, reset=Style.RESET_ALL))
    ...
    def ThreadedAssociation(self, a):
        ...
        if a.threadprivate:
            ...
            # self.fw('endif')
# pcopy__ must be copied in so eahc thread has an allocated target for the
# pointer. Note that deallocation called after the parallel construct only
# deallocte pcopy__ for the master thread but not for the other threads.
# That way, the py array object corresponding to the memory allocated in
# the subroutine for the master thread can be freed. But we do not care
# about memory allocated in the other threads since the pyarray object has
# not link to these memory locations. Note that the pointer and the target
# are persistent between references to parallel constructs for each
