...
parser = optparse.OptionParser(
                   usage="Forthon [options] pkgname [extra Fortran or C files to be compiled or objects to link] [options for setuptools]",
                   description="""
...
# --- extra files, that are passed to setuptools during the call to setup.
...
parser.add_option('--cargs', help='Additional options for the C compiler. These are passed through setuptools, which does the compilation of C code. If there are any spaces in options, it must be surrounded in double quotes.')
